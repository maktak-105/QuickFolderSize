...
- MFT高速経路（管理者実行）で、CLI出力とJSON/Markdownレポートの`path`が全ノード空になっていた不具合を修正。
- MFT高速経路で、`$ATTRIBUTE_LIST`を持つファイル（断片化の激しい大きいファイル等）のサイズが0になっていた不具合を修正。C:\で約1万ファイル・25GBが計上漏れしていた。同じ原因で起きていた、8.3短縮名での表示と、名前が拡張レコードにしか無いファイルの欠落も解消。
- OneDriveなどのクラウドフォルダが、再解析ポイントであるためにツリーから丸ごと除外されていた不具合を修正。除外するのはジャンクション・シンボリックリンク等（名前サロゲート）だけにした。オンラインのみのファイルは、ファイル数に含めサイズは0とする。
- 修正後、C:\の合計は`fsutil volume allocationReport`のユーザーファイル割り当てとほぼ一致（181.56GB 対 182.0GB、1,300,498件 対 1,300,477件）。
